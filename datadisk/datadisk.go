...
func (d datadisk) ChangeDevice(newDevice string) (bool, *dbus.Error) {
	...
	dataDevice, err := udisks2helper.GetRootDeviceFromLabel("sasos-data")
	if err != nil {
	...
	err = udisks2helper.PartitionDeviceWithSinglePartition(newDevice, linuxDataPartitionUUID, "sasos-data-external")
	if err != nil {
		return false, dbus.MakeFailedError(err)
	}

	/* Move request marker for sasos-data.service */
...
const (
	objectPath = "/org/saserver/os/DataDisk"
	ifaceName  = "org.saserver.os.DataDisk"
)
